...
import { Plus, Search, Music, Mail, Phone, Euro, Edit, Trash2, MoreVertical, LayoutGrid, List, Users, ChevronDown, PhoneCall, MessageSquare } from "lucide-react";
    ...
    return (
      <Card
        ...
        <CardContent className="pt-0 space-y-2">
          {/* Immer sichtbar: Telefon klickbar */}
          {musiker.telefon &&
          <div className="relative">
              <button
              onClick={(e) => { e.stopPropagation(); setPhoneMenuId(phoneMenuId === musiker.id ? null : musiker.id); }}
              className="flex items-center gap-2 text-sm text-blue-600 hover:text-blue-800 transition-colors">
                <Phone className="w-4 h-4" />
                <span>{musiker.telefon}</span>
              </button>
              {phoneMenuId === musiker.id &&
            <>
                ...
                <div className="absolute left-0 top-full mt-1 bg-card border border-border rounded-lg shadow-lg z-50 w-52 overflow-hidden">
                  <a href={`tel:${musiker.telefon}`} onClick={(e) => { e.stopPropagation(); setPhoneMenuId(null); }}
                  ...
                  <a href={`sms:${musiker.telefon}`} onClick={(e) => { e.stopPropagation(); setPhoneMenuId(null); }}
                  ...
                  <a href={`https://wa.me/${musiker.telefon.replace(/[^0-9+]/g, '')}`} target="_blank" rel="noreferrer" onClick={(e) => { e.stopPropagation(); setPhoneMenuId(null); }}
                  className="flex items-center gap-3 px-4 py-3 hover:bg-muted transition-colors text-sm text-foreground border-t">
      ...
      <div
        ...
        <div className="w-12 h-12 rounded-full flex-shrink-0 overflow-hidden">
          {musiker.profilbild_url ? (
            ...
            <div className="bg-[rgb(var(--primary))] text-white font-bold w-full h-full flex items-center justify-center">
              {initials}
            </div>
          )}
        </div>
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-2">
            <h3 className="font-semibold text-lg text-foreground truncate">{musiker.name}</h3>
            {musiker.prioritaet && (
              <span style={{ ...getPrioritaetStyle(musiker.prioritaet), padding: '2px 8px', borderRadius: '9999px', fontSize: '12px', fontWeight: 'bold' }}>
                {musiker.prioritaet}
              </span>
      ...
      <div className="min-h-screen bg-background p-3 md:p-8 overflow-x-hidden">
        <div className="w-full max-w-7xl mx-auto">
          <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-8">
            <div>
              <h1 className="text-3xl md:text-4xl font-bold text-foreground mb-2">Musiker</h1>
              <p className="text-muted-foreground">Verwalte dein Musiker-Portfolio</p>
            </div>
            <Button
              onClick={() => {
                setEditingMusiker(null);
